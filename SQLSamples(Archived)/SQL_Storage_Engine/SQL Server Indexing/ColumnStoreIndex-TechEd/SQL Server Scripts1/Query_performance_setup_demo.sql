...
If db_id('ColumnstoreDemos_queryperformance') is null
	Create DATABASE ColumnstoreDemos_queryperformance
GO
...
SELECT TOP 1000
			...
			((a1.reserved + ISNULL(a4.reserved,0))* 8)/1024 AS "reserved(In MB)" ,
			(a1.data * 8)/1024 AS "data(In MB)" ,
			((CASE WHEN (a1.used + ISNULL(a4.used,0)) > a1.data 
			THEN (a1.used + ISNULL(a4.used,0)) - a1.data ELSE 0 END) * 8)/1024 AS "index_size(In MB)" ,
			((CASE WHEN (a1.reserved + ISNULL(a4.reserved,0)) > a1.used THEN 
			(a1.reserved + ISNULL(a4.reserved,0)) - a1.used ELSE 0 END) * 8)/1024 AS "unused(In MB)" 
			FROM
			(
			SELECT
			ps.object_id,
...
GO
 ...
 Create NonClustered Index NC1 on SalesOrderDetail (OrderQty, UnitPrice, UnitPriceDiscount)
with (Data_Compression = PAGE)